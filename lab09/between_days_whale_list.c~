...
void print_between(struct pod *first_pod, struct date *start_day, struct date *finish_day);
...
//
void between_days_whales(struct pod *first_pod, struct date *start_day, struct date *finish_day) {
    print_between(first_pod, start_day, finish_day);
    // PUT YOUR CODE HERE
...
void print_between(struct pod *first_pod, struct date *start_day, struct date *finish_day) {
    ...
    struct date *date1 = start_day;
    struct date *date2 = finish_day;
    
    while (p != NULL) {
        if ((p->when->year == date1->year) && (p->when->year == date2->year)) {
            if ((p->when->month == date1->month) && (p->when->month == date2->month)) {
                if ((p->when->day >= date1->day) && (p->when->day <= date2->day)) {
                    printf("%02d/%02d/%02d %d %s\n", p->when->day, p->when->month, p->when->year, p->how_many, p->species);
                }
            } else if (date1->month < date2->month) {
                if (p->when->month == date1->month && p->when->day >= date1->day) {
                    printf("%02d/%02d/%02d %d %s\n", p->when->day, p->when->month, p->when->year, p->how_many, p->species);
                } else if (p->when->month == date2->month && p->when->day <= date2->day) {
                    printf("%02d/%02d/%02d %d %s\n", p->when->day, p->when->month, p->when->year, p->how_many, p->species);
                } else if (p->when->month > date1->month && p->when->month < date2->month) {
                    printf("%02d/%02d/%02d %d %s\n", p->when->day, p->when->month, p->when->year, p->how_many, p->species);
                }
            }
        } else if ((p->when->year >= date1->year) && (p->when->year <= date2->year)) {
            if ((date1->month <= 12 && date1->month <= p->when->month) && p->when->year == date1->year) {
                if (p->when->month == date1->month && p->when->day >= date1->day) {
                    printf("%02d/%02d/%02d %d %s\n", p->when->day, p->when->month, p->when->year, p->how_many, p->species);
                } else if (p->when->month > date1->month ) {
                    printf("%02d/%02d/%02d %d %s\n", p->when->day, p->when->month, p->when->year, p->how_many, p->species);
                }
            } else if (p->when->month <= date2->month && p->when->year == date2->year) {
                if (p->when->month < date2->month) {
                    printf("%02d/%02d/%02d %d %s\n", p->when->day, p->when->month, p->when->year, p->how_many, p->species);
                } else if (p->when->month == date2->month && p->when->day <= date2->day) {
                    printf("%02d/%02d/%02d %d %s\n", p->when->day, p->when->month, p->when->year, p->how_many, p->species);
                }
