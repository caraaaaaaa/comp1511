...
void print_whales(struct pod *first_pod, struct date *day);
...
void print_whales(struct pod *first_pod, struct date *day) {
    ...
    struct date *date1 = day;
    
    while (p != NULL) {
        if (p->when->day == date1->day) {
            if (p->when->month == date1->month) {
                if (p->when->year == date1->year) {
